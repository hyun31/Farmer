...
<body>
		...
		<div class="container" role="main" align="left">
			...
			<div class="bg-white rounded shadow-sm">
				...
				<div class="board_article_nm">
					<c:out value="${hp001_d004Content.article_nm}" />
				</div>
				
				<div class="board_info_box">
					<span class="board_author"><c:out value="${hp001_d004Content.writer_id}" />,</span>
					<span class="board_date"><c:out value="${hp001_d004Content.reg_date}" /></span>
					
					<span class="board_question"><c:out value="${hp001_d004Content.question_type}"/></span>
					<span class="board_order"><c:out value="${hp001_d004Content.order_number}"/></span>
					<span class="board_email"><c:out value="${hp001_d004Content.email}"/></span>
					
					
				</div>
				<div class="board_article_content">${hp001_d004Content.article_content}</div>
			   </div>
			...
			<!-- Reply Form {s} --> 
 			<div class="my-3 p-3 bg-white rounded shadow-sm" style="padding-top: 10px"> 
 				<form:form name="form" id="form" role="form" modelAttribute="reply4VO" method="post"> 
	...
	$(document).on('click','#btnDelete', function(){
		var url = "${pageContext.request.contextPath}/hp/deleteBoard4";
		url = url + "?article_no=" + ${hp001_d004Content.article_no};
		location.href = url;
	});
	// 수정 버튼 클릭 이벤트  -- 1007 18:43
	$(document).on('click', '#btnUpdate', function(){
		var url = "${pageContext.request.contextPath}/hp/editForm4";
		url = url + "?article_no="+${hp001_d004Content.article_no};
		url = url + "&mode=edit";
		location.href = url;
	});
	...
	$(document).on('click', '#btnReplySave', function(){
		...
		var paramData = JSON.stringify({"board_content": replyContent
				, "reg_id": replyReg_id
				, "bid":'${hp001_d004Content.bid}'
		});
		var headers = {"Content-Type" : "application/json"
				, "X-HTTP-Method-Override" : "POST"};
		$.ajax({
			url: "${pageContext.request.contextPath}/restBoard4/saveReply"
			, headers : headers
			, data : paramData
			, type : 'POST'
			, dataType : 'text'
	...
	function fn_updateReply(rid, reg_id){
		...
		var headers = {"Content-Type" : "application/json"
				, "X-HTTP-Method-Override" : "POST"};
		$.ajax({
			url: "${pageContext.request.contextPath}/restBoard4/updateReply"
			, headers : headers
			, data : paramData
			, type : 'POST'
			, dataType : 'text'
	...
	function fn_deleteReply(rid){
		...
		$.ajax({
			url: "${pageContext.request.contextPath}/restBoard4/deleteReply"
			, data : paramData
			, type : 'POST'
			, dataType : 'text'
			, success: function(result){
	...
	function showReplyList(){
		var url = "${pageContext.request.contextPath}/restBoard4/getReplyList";
		var paramData = {"bid" : "${hp001_d004Content.bid}"};
