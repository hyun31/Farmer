...
<script>
	...
	$(document).ready(function(){
		...
		if (mode == 'edit'){
			  ...
			  $("input:hidden[name='article_no']").val('<c:out value="${hp001_d001Content.article_no}"/>');
			  $("input:hidden[name='mode']").val('<c:out value="${mode}"/>');
			  $("#writer_id").val('<c:out value= "${hp001_d001Content.writer_id}"/>');
			  $("#article_nm").val('<c:out value= "${hp001_d001Content.article_nm}"/>');
			  $("#article_content").val('<c:out value= "${hp001_d001Content.article_content}"/>');
...
<body>
	<article>
		<div class="container" role="main" >
			<br><h5>공지사항게시판 작성하기</h5>
			
			<form:form  name="form" id="form" role="form" modelAttribute="hp001_d001VO" method="post"
				action="${pageContext.request.contextPath}/hp/saveBoard1">
