...
<body>
	<div id="wrapper">
		<div>
			<h4 style="text-align: left;"><strong>연장신청 내역</strong></h4>
		</div>
		...
		<table id="farmTable" class="table table-bordered" style="margin-bottom: 0;">
			<thead>
				<tr>
					<th style="border-top: 2px solid #343a40;">번호</th>
					<th style="border-top: 2px solid #343a40;">농장명</th>
					<th style="border-top: 2px solid #343a40;">농장주소</th>
					<th style="border-top: 2px solid #343a40;">텃밭명</th>
					<th style="border-top: 2px solid #343a40;">연장기간</th>
				</tr>
			</thead>
			<tbody>
				<tr>
					<td style="border-bottom: 2px solid #343a40;">1</td>
					<td style="border-bottom: 2px solid #343a40;">${extensionInfo.farm_nm }</td>
					<td style="border-bottom: 2px solid #343a40;">${extensionInfo.farm_address }</td>
					<td style="border-bottom: 2px solid #343a40;">${extensionInfo.garden_nm } ${extensionInfo.garden_no }</td>
					<td style="border-bottom: 2px solid #343a40;">${extensionInfo.extension_date }개월</td>
				</tr>
			</tbody>
		</table>
		...
		<div id="price">
			<h6>
			결제금액 :
			<c:if test="${extensionInfo.extension_date == 3 && extensionInfo.area_size == 01}">
				60,000원
			</c:if> 
			<c:if test="${extensionInfo.extension_date == 6 && extensionInfo.area_size == 01}">
				80,000원
			</c:if>
			<c:if test="${extensionInfo.extension_date == 3 && extensionInfo.area_size == 02}">
				100,000원
			</c:if> 
			<c:if test="${extensionInfo.extension_date == 6 && extensionInfo.area_size == 02}">
				120,000원
			</c:if> 
			</h6>
		</div>
			...
			<div id="collapse1" class="card-collapse collapse show" role="tabpanel">
				<div class="card-block" style="padding-top: 10px;">
					<form action="${contextPath }/ep/ep001_d007/kakaoPay" method="post" id="form">
						<input type="hidden" name="farm_no" value="${extensionInfo.farm_no }">
						<input type="hidden" name="farm_nm" value="${extensionInfo.farm_nm }">
						<input type="hidden" name="garden_no" value="${extensionInfo.garden_no }">
						<input type="hidden" name="garden_nm" value="${extensionInfo.garden_nm }">
						<input type="hidden" name="contract_term" value="${extensionInfo.extension_date }">
						<input type="hidden" name="area_size" value="${extensionInfo.area_size }">
						<input type="hidden" name="order_code" value="02">
						<input type="hidden" name="quantity" value="1">
						<div class="form-group row">
						<label class="col-sm-2 col-form-label-sm">주문자</label>
						<input type="text" id="user_nm" name="user_nm" class="form-control form-control-sm col-sm-3" value="${user.USER_NM }" maxlength="4" placeholder="이름">
						</div>
						<div class="form-group row">
							<label class="col-sm-2 col-form-label-sm">이메일</label>
							<input type="text" class="form-control form-control-sm col-sm-5" name="email" id="email" value="${user.USER_EMAIL }">
						</div>
						<div class="form-group row">
							<label class="col-sm-2 col-form-label-sm">휴대전화</label>
								<input type="text" class="form-control form-control-sm col-sm-5" name="phone" id="phone" value="${user.USER_PHONE }"> 
						</div>
						<div class="form-group row">
		...
		<div style="text-align: center; margin: 40px;">
			<button class="btn btn-sm" type="button" onclick="window.location.href='${contextPath}/ep/ep001_d003/init.do'" style="background-color: #f4623a; color:white;">뒤로가기</button>
